...
var db = ravendb(settings.server, settings.db);
var RavenDB = function(settings) {
  RavenDB.super_.call(this, name, settings);

  if(!settings || !settings.server) {
    throw Error('RavenDB ctor: expected valid settings');
  }
};
util.inherits(RavenDB, Connector);

RavenDB.prototype.connect = function(done) {
  done();
};
